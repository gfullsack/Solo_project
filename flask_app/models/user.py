...
from flask_app.models import tea
...
class User:
    ...
    def get_my_tea(cls,data):
        query = "SELECT * FROM users LEFT JOIN teas on users.id = teas.user_id WHERE users.id = %(id)s;"
        results = connectToMySQL(cls.db_name).query_db(query,data)
        ...
        for row in results:
            ...
            user_teas = tea.Tea(tea_data)
            one_user.teas.append(user_teas)
        return one_user
